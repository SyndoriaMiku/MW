...
def _active_battle_for(character):
...
def get_active_battle(request):
    """Return the caller's active battle, or 204 when there is none."""
    ...
    combat = _active_battle_for(character)
    if combat is None:
...
def player_action(request, combat_id):
    ...
    with transaction.atomic():
        ...
        try:
            target_query = {'id': target_id} if target_id is not None else {'position': target_position}
            target = combat.combatants.get(**target_query)
        except Combatant.DoesNotExist:
